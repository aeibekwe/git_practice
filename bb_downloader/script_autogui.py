...
class User:
    ...
    def got_to_extra_content_page(self, course):  # this is basically just for the assignments page in compsci
        if course in self.courses:
            buttons = self.courses.get(course)
            if buttons[2]:
                click(buttons[2])
...
scroll_down()  # scrolls to bottom of 'My Courses', so you can see all the courses
...
# Click content button (should be in the same location for each course)
